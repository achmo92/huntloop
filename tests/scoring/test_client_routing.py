...
from huntloop.llm.client import (
    CodexGatewayClient,
    LlmResponseError,
    complete_json,
    get_llm_client,
...
def test_get_llm_client_codex_gateway_is_explicit(credentials_session, monkeypatch):
    monkeypatch.setenv("HUNTLOOP_LLM_PROVIDER", "codex_gateway")
    monkeypatch.setenv("HUNTLOOP_OPENAI_BASE_URL", "http://localhost:8787/v1")
    monkeypatch.setenv("HUNTLOOP_OPENAI_API_KEY", "dummy")

    client = get_llm_client(credentials_session)

    assert isinstance(client, CodexGatewayClient)
    assert str(client.base_url) == "http://localhost:8787/v1/"
...
def test_complete_json_success():
    ...
    client.chat.completions.responses.append(
        FakeResponse('{"name": "test", "score": 5}', pt=20, ct=10)
    )
...
def test_codex_gateway_complete_json_preserves_contract():
    transport = FakeClient()
    transport.chat.completions.responses.append(
        FakeResponse('{"name": "test", "score": 5}', pt=20, ct=10)
    )

    call = complete_json(
        CodexGatewayClient(transport),
        model="codex",
        system="s",
        user="u",
        schema=DummySchema,
    ...
    assert call.content == {"name": "test", "score": 5}
    assert call.usage.prompt_tokens == 20
    assert transport.chat.completions.kwargs["model"] == "codex"
...
def test_complete_json_no_retry_on_bad_request():
    ...
    client.chat.completions.responses.append(
        openai.BadRequestError("bad", response=response, body=None)
    )
...
def test_openai_import_is_confined_to_llm_client():
    ...
    for py_file in src_dir.rglob("*.py"):
        ...
        for line in text.splitlines():
            line = line.strip()
            if line.startswith(("import openai", "from openai")):
