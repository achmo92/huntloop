...
class CodexGatewayClient:
    """Explicit opt-in boundary for Codex-only behavior.

    It delegates today's single-call behavior to the unchanged OpenAI-compatible
    transport. Future batching belongs on this type, never in the default path.
    """

    provider = "codex_gateway"

    def __init__(self, transport: openai.OpenAI):
        self.transport = transport

    def complete_json(self, **kwargs) -> LlmCall:
        return _complete_openai_json(self.transport, **kwargs)

    def __getattr__(self, name):
        return getattr(self.transport, name)


def get_llm_client(credentials_session) -> openai.OpenAI | CodexGatewayClient:
    cfg = load_config()
    ...
    if cfg.llm_provider == "codex_gateway":
        return CodexGatewayClient(transport)
    return transport
...
def complete_json(
    ...
    *,
    ...
    if isinstance(client, CodexGatewayClient):
