...
import huntloop.db.models  # noqa: F401  -- registers all nine tables on Base.metadata
...
# Guard against Pitfall C: this env.py must never see the credentials table,
# and must never be missing a main-store table. A silent mismatch here
...
if _actual != EXPECTED_TABLES:
    raise RuntimeError(
        "migrations/main/env.py is bound to the wrong metadata. "
